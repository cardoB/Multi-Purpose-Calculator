...
Polynomial getPolynomial(char variable)
{
    ...
    degree = getPositiveInt();
    degree++;


    cout << "Enter the coefficients, starting with the constant and moving up each term. For missing terms, enter zero: ";
    double* coefficients = new double[degree];

    for (int i = 0; i < degree; i++)
    {
        coefficients[i] = getReal();
    }
